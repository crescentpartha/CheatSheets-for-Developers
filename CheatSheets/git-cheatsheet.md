...
| `git reset HEAD "name_of_file"` | Go back to the pointer I'm already at, remove from staged |
| `git reset --soft "hash"` | Take my changes and go back/kill the commit I made but the file will be staged with the change ready to be committed again |
| `git reset --mixed "hash"` | Return/Kill the commit too but it returns the files to before staged i.e. modified but it will still return |
| `git reset --hard "hash"` | It will simply ignore the existence of this commit and everything that was done in it, that is, it is a very rough reset and it will undo everything that was done in the commit |
